...
data_dir=${data_path}/{{ cookiecutter.pipeline_name.lower().replace(' ', '-') }}_test_set
...
then
    ...
    curl https://your_website.com/{{ cookiecutter.pipeline_name.lower().replace(' ', '-') }}_test_set.tar.bz2 > ${data_path}/{{ cookiecutter.pipeline_name.lower().replace(' ', '-') }}_test_set.tar.bz2
    echo "Unpacking test set..."
    tar xvjf ${data_path}/{{ cookiecutter.pipeline_name.lower().replace(' ', '-') }}_test_set.tar.bz2 -C ${data_path}
    echo "Done"
fi

# Run name
run_name="Test {{ cookiecutter.pipeline_name }} Run: "$(date +%s)
